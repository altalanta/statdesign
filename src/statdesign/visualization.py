...
def plot_power_curves(
    ...
    if highlight_current:
        ...
        if min_val <= current_val <= max_val:
            ...
            ax.plot(current_val, current_power, 'ro', markersize=8, 
    ...
    for i in range(len(power_values) - 1):
        if (power_values[i] >= 0.80 and power_values[i+1] < 0.80) or \
           (power_values[i] < 0.80 and power_values[i+1] >= 0.80):
            ...
            x_cross = np.interp(0.80, [power_values[i], power_values[i+1]], 
                              [valid_sweep_values[i], valid_sweep_values[i+1]])
            power_80_crossings.append(x_cross)
    
    # Annotate crossings
    for x_cross in power_80_crossings:
        ax.annotate(f'{sweep_param}={x_cross:.3f}', 
                   xy=(x_cross, 0.80), xytext=(10, 10),
    ...
    if return_data:
        data = pd.DataFrame({
            sweep_param: valid_sweep_values,
            'power': power_values
        })
        return fig, data
